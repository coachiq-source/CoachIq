...
Optional overrides:
    ...
    FORMSPREE_SECRET_WATERPOLO      HMAC signing secret for the water polo form
                                    (weekly + game-prep + post-game all submit
                                    to one Formspree form; the webhook branches
                                    on `formType`).
...
class Settings:
    ...
    # Each sport uses a SINGLE Formspree form for every form type that sport
    # supports — weekly, game-prep, and (water polo) post-game all submit to
    # the sport's one form, and the webhook branches on `formType` server-
    # side to dispatch to the right pipeline. The signing secret is
